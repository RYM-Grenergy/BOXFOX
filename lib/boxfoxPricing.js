...
export function calculateBoxPrice(params, customConfigs = null) {
  ...
  const Z2 = totalDieCost; 

  // Step 4: Fixed Charges (AE2) - Includes Designing and Die Cost
  // Based on Python reference, designing is a fixed cost. 
  // We also include window, leafing and pasting as fixed setups if they exist in spec.
  const AE2 = ru(design + Z2 + leafing + window_);

  // Step 5: Other Charges (AD2)
  // Python: roundup((qty * pasting) * 1.05 + max(sheetQty * ups * addonRate, 200) + 1)
  const addonDef = ADDON_OPTIONS[addon];
  const addonRate = addonDef?.rate || 0;
  const AD2 = ru((qty * pasting * 1.05) + Math.max(sheetQty * ups * addonRate, 200) + 1);

  // Step 6: Paper Cost (P2)
  const P2 = calcPaper(W, H, sheetQty, gsm, cc5);

  // Step 7: Base Per Unit (AG2)
  // Note: Python script EXCLUDES die_cost from per-unit price in comparison columns, 
  // but we include it here as it's part of the job cost for this specific order.
  ...
  // If the addon type is 'lam', it uses the same formula as lamination.
  ...
  return {
    ...
    breakdown: {
      ...
      pasting: (qty * pasting * 1.05),
      setup: Math.max(sheetQty * ups * addonRate, 200) + 1,
      design: design,
      die: totalDieCost,
    },
