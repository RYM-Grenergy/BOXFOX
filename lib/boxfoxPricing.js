...
export function unitPriceFromThreePoints({ priceAt1, priceAt100, priceAt500 }, qty) {
  ...
  if (q <= 1) return p1;
  if (q <= 100) {
    // interpolate between (1, p1) and (100, p100)
    const t = (q - 1) / (100 - 1);
    return p1 + (p100 - p1) * t;
  }
  if (q <= 500) {
    // interpolate between (100, p100) and (500, p500)
    const t = (q - 100) / (500 - 100);
    return p100 + (p500 - p100) * t;
  }
  // q > 500: extrapolate using slope from 100->500
  const slope = (p500 - p100) / (500 - 100);
  return p500 + slope * (q - 500);
}
