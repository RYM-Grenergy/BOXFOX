 ...
 * @param {Object} [customConfigs]   - Optional overrides for rates/tables
 *
...
export function calculateBoxPrice(params, customConfigs = null) {
  ...
  // Use dynamic configs if provided, else fallback to hardcoded defaults
  const markupTable = customConfigs?.MARKUP_TYPES || MARKUP_TYPES;
  ...
  // Step 3: Die Rate (Z2) - used in AE2 as a rate, not total run cost
  const dieRatePerSheet = sheetQty <= 5080 ? 0.25 : 0.20;
  const totalDieCost = dieCutting ? ru(dieRatePerSheet * sheetQty) : 0;

  // Step 4: Fixed Charges (AE2)
  // Formula: ROUNDUP(designing + dieRatePerSheet + leafing + pasting + window_)
  // Note: We use the rate here to match the user's breakdown (e.g. 100 + 0.25 + 4 -> 105)
  const AE2 = ru(design + (dieCutting ? dieRatePerSheet : 0) + leafing + pasting + window_);

  // Step 5: Other Charges (AD2)
  // Formula: ROUNDUP(qty * window * 1.05 + MAX(sheetQty * ups * (pasting + addonRate), 200) + 1)
  const addonDef = ADDON_OPTIONS[addon];
  const addonRate = addonDef?.rate || 0;
  const AD2 = ru((qty * window_ * 1.05) + Math.max(sheetQty * ups * (pasting + addonRate), 200) + 1);

  // Step 6: Paper Cost (P2)
  const P2 = calcPaper(W, H, sheetQty, gsm, cc5);

  // Step 7: Base Per Unit (AG2)
  // Formula: (P2 + AE2 + X2 + AD2) / qty
  ...
  // Step 9: Add-On Per Unit (Optional extra processing)
  let extraAddonPerUnit = 0;
  if (addonDef && addonDef.type === 'lam' && addon !== 'Plain') {
    extraAddonPerUnit = calcLam(W, H, sheetQty, qty, addon);
  }

  // Step 10: Subtotal
  const subtotalPerUnit = (AG2 + lamPerUnit + extraAddonPerUnit) * dblChg;
  ...
  return {
    ...
    dieRate: dieRatePerSheet,
    dieCost: totalDieCost,

    // Per-unit breakdown
    basePerUnit: AG2,
    lamPerUnit,
    addonPerUnit: extraAddonPerUnit,
    subtotalPerUnit,
    markup,
    ...
    breakdown: {
      ...
      fixed: AE2,
      other: AD2,
      lamination: lamPerUnit * qty,
      addon: extraAddonPerUnit * qty,
      windowPerUnitCharge: (qty * window_ * 1.05),
      pastingSheetCharge: Math.max(sheetQty * ups * (pasting + addonRate), 200) + 1,
      design: design,
      die: totalDieCost,
    },
 ...
 * price at q=1, q=100 and q=500. Uses piecewise-linear interpolation
 * between (1, p1) -> (100, p100) and (100, p100) -> (500, p500).
 * For q > 500 it linearly extrapolates using the slope from 100->500.
 * Returns a number (per-unit price) or null if insufficient data provided.
 */
export function unitPriceFromThreePoints({ priceAt1, priceAt100, priceAt500 }, qty) {
  const p1 = Number(priceAt1 || 0);
  const p100 = Number(priceAt100 || 0);
  const p500 = Number(priceAt500 || 0);

  if (!p1 || !p100 || !p500) return null;

  const q = Math.max(1, Number(qty) || 1);

  if (q <= 1) return p1;

  // Use power-law interpolation: P = a * Q^b
  // This is more "proper" for pricing than linear interpolation
  // as it ensures prices never go negative and follow a natural decay curve.

  const calcPowerPrice = (q1, p1, q2, p2, qTarget) => {
    if (p1 === p2) return p1;
    const b = Math.log(p2 / p1) / Math.log(q2 / q1);
    const a = p1 / Math.pow(q1, b);
    return a * Math.pow(qTarget, b);
  };

  if (q <= 100) {
    return calcPowerPrice(1, p1, 100, p100, q);
  }

  if (q <= 500) {
    return calcPowerPrice(100, p100, 500, p500, q);
  }

  // For q > 500, continue the power curve from the 100-500 segment
  // but ensure it doesn't drop too aggressively or go below a reasonable floor
  // (using a floor of 80% of p500 as a safety measure for extreme quantities)
  const floor = p500 * 0.7;
  const extrapolated = calcPowerPrice(100, p100, 500, p500, q);
